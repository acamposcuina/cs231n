...
def relu_forward(x):
    ...
    # maximum devuelve un array con el maximo entre los dos arrays que le pasas
    # elemento a elemento
    out = np.maximum(0., x)
...
def relu_backward(dout, cache):
    ...
    # dx = dout * dz_dx (siendo z = relu_forward(x))  
    dx = dout * (x > 0)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = {'x':x, 'mean':sample_mean, 'var':sample_var, 'gamma':gamma, 'z':z}
...
def batchnorm_backward(dout, cache):
    ...
    dzdx = 1 / np.sqrt(cache['var'])                                #[NxD]
    dzdu = -1 / np.sqrt(cache['var'])                               #[1xD]
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    sample_var = np.var(x_t, axis = 0) + eps

    z = (x_t - sample_mean) / np.sqrt(sample_var)
    ...
    cache = {'x':x, 'gamma':gamma, "var": sample_var, "beta": beta, 'z':z}
...
def layernorm_backward(dout, cache):
    ...
    dbeta = np.sum(dout_t, axis=1)
    dgamma = np.sum(dout_t * cache['z'], axis=1)

    z = cache['z']
    dfdz = dout_t * cache['gamma']                                     #[NxD]
    dfdz_sum = np.sum(dfdz, axis=0)                                  #[1xD]
    dx = dfdz - dfdz_sum/N - np.sum(dfdz * z, axis=0) * z/N          #[NxD]
    dx /= np.sqrt(cache['var'])
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Añado el zero-padding
    x_pad = np.pad(x, ((0,0), (0,0), (pad,pad), (pad,pad)), 'constant')
    H_pad = x_pad.shape[2]
    W_pad = x_pad.shape[3]

    # Creo el tensor de salida a ceros
    out = np.zeros((N, F, H_prima, W_prima), dtype = float)
   
    # Ejemplo: out[0,0,0] = np.sum(X[:5,:5,:] * W0) + b0
      ...
      for f in range(F):
        ...
        for i in range(0, H_pad-HH + 1, stride):
          ...
          for j in range(0, W_pad-WW + 1, stride):
            out[n, f, i_aux, j_aux] = np.sum(x_pad[n, :, i:i+HH, j:j+WW] * w[f]) + b[f]
            j_aux += 1
          i_aux += 1
        ...
        for c in range(C):
            for i in range(H_prime):
                for j in range(W_prime):
                    ...
                    # Los gradientes pasan solo por los índices de max_pool
